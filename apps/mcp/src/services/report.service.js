...
export class ReportService {
  ...
  static async grossProfit({ startDate, endDate } = {}) {
    await connectDB();
    const match = {};
    if (startDate || endDate) {
      match.order_date = {};
  ...
  static async cogs({ startDate, endDate } = {}) {
    await connectDB();
    const match = {};
    if (startDate || endDate) {
      match.order_date = {};
  ...
  static async expenseReport({ startDate, endDate } = {}) {
    await connectDB();
    const match = {};
    if (startDate || endDate) {
      match.createdAt = {};
  ...
  static async returns({ startDate, endDate } = {}) {
    await connectDB();
    const match = { type: "return" };
    if (startDate || endDate) {
      match.createdAt = {};
      if (startDate) match.createdAt.$gte = new Date(startDate);
      if (endDate) match.createdAt.$lte = new Date(endDate);
