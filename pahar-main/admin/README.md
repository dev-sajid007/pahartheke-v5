...
This admin panel manages the landing page content displayed on the customer-facing storefront (port 3000). It directly updates the **pahar-main backend** (`localhost:5000`) which persists data in MongoDB.
...
The admin panel uses the same backend as the main storefront (`BACKEND_API_URL`). Configure in `frontend/.env.local`:
